...
def main():
    ...
    rc = {}
    rc['true T'] = True_T
    rc['initial T'] = T.cpu().numpy()
    for ke in rc:
        with open(record_file, 'a', newline='') as csvFile:
            writer = csv.writer(csvFile)
            writer.writerow(np.array([ke,]))
            for roww in range(rc[ke].shape[0]):
                writer.writerow(np.array(rc[ke][roww, :]))
    csvFile.close()
    ...
    for epoch in range(args.n_epoch_revision):
        ...
        if epoch == (args.n_epoch_revision-1):
            with open(record_file, 'a', newline='') as csvFile:
                writer = csv.writer(csvFile)
                writer.writerow(np.array(['(T+correction)', ]))
                for roww in range((T+correction).cpu().detach().numpy().shape[0]):
                    writer.writerow(np.array((T+correction).cpu().detach().numpy()[roww, :]))
            csvFile.close()
